package oopExam;
/*
 * Movable   Attackable             Object  (모든 객체는 Object의 자손)
 * 
 * 
...
interface Fightable extends Movable, Attackable {} // 인터페이스에 타 인터페이스를 다중상속하여 넣음
...
public class InterfaceExam {
	...
	public static void main(String[] args) {
		...
		if(f instanceof Unit) { // 관계가 있는 객체니 -> true
			System.out.println("f는 Unit의 자손입니다.");
		}
		
		if(f instanceof Fightable) { // 상속의 관계를 가진 객체니
...
/* 인터페이스 interface
	인터페이스는 일종의 추상 클래스이다. 인터페이스는 추상 클래스처럼 추상 메서드를 갖지만 추상 클래스보다 추상화의 정도가 높아
	추상클래스와는 달리 일반 메서드를 구성원으로 가질 수 없다. 오직 추상메서드와 상수만을 멤버로 가질 수 있다.
	그 외의 어떤 다른 요소도 허용하지 않는다.
	추상 클래스는 부분적으로만 완성된 '미완성 설계도'라고 한다면 인터페이스는 구현된 것은 아무것도 없고 밑그임만 그려져 있는
	'기본 설계도'라고 할 수 있다. 다른 클래스에 도움을 주려는 목적으로 사용된다.
	...
	interface Play {
		...
		인터페이스 상속
		인터페이스는 인터페이스로부터 상속을 받을 수 있다 클래스와는 다르게 다중 상속 즉 여러 개의 인터페이스로부터 상속 가능.
		...
		추상 클래스가 상속을 통해 추상메서드를 완성한 것처럼 인터페이스도
		자신의 정의된 추상 메서드의 몸통을 만들어 주는 클래스를 작성해야 한다.
		
		인터페이스의 다형성
		클래스에서의 다형성도 자손 클래스의 객체를 조상 타입의 참조변수로 참조하는 것이 가능하다는 것을 보았다.
		인터페이스 역시 인터페이스 타입의 참조 변수로 이를 구현한 클래스의 인스턴스(객체)를 참조할 수 있고 형변환도 가능하다.
		
		Fightable f = new Fighter(); // Fighter가 Fightable 인터페이스를 구현하고 있음으로 사용 가능
		
		따라서 인터페이스는 메서드의 매개변수 타입으로 사용될 수 있다.
		
		void attack(Fightable f) // 객체인데 인터페이스를 자료형으로 쓸 수 있다.
		...
		: 서로 상속관계도 아니고 같은 조상 클래스도 가지지 않은 아무 관련 없는 클래스들에게
		    하나의 인터페이스를 공통적으로 구현해 관계를 만들 수 있다.
		    
		- 독립적인 프로그래밍이 가능하다.
		: 인터페이스를 이용하면 선언과 구현을 분리시킬 수 있기 때문에 실제 구현에 독립적인 프로그램을 작성하는 것이 가능하다.
		    클래스 간의 직접적인 관계를 인터페이스를 이용하여 간접적인 관계로 변경하면 한 클래스의 변경이 관련된 
		    다른클래스에 영향을 미치지 않는 독립적인 프로그래밍이 가능해진다.
