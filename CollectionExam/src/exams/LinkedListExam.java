...
/* 배열은 가장 기본적인 형태의 자료구조로 그 구조가 간단하고 사용하기 쉽고 가장 빠르다는 장점을 가지고 있으나 단점이 있다.
 * 
 * 1. 크기를 변경할 수 없다.
 ...
 * 배열은 모든 데이터가 연속적으로 존재해야 하지만 링크드 리스트는 불연속적으로 존재하는 데이터를 서로
 * 연결linked 한 형태로 구성된다.
 * 
 * array       0 1 2 3 4
 * 
 ...
 *              0    1    2        3
 *             ->   ->   1->3      ->
 *             새로운 데이터를 추가할 때는 새로운 요소를 생성한 다음 추가하고자 하는 위치의 이전 요소의 참조를
 *             새로운 요소에 대한 요소에 대한 참조로 변경해주고 새로운 요소가 그 다음 요소를 참조하도록 변경하기만
 *             하면 되므로 처리속도가 매우 빠르다.
 *             
 *             링크드 리스트는 이동 방향이 단방향이기 때문에 다음 요소에 대한 접근은 쉽지만 이전 요소에 대한 접근이 어렵다.
 *             이러한 단점을 보완한 더블링크드리스트도 있다.
 *    
 * 순차적으로 추가/삭제하는 경우에는 ArrayList LinkedList보다 빠르다.
 * 중간에 데이터를 추가/삭제하는 경우네는 LinkedList가 ArrayList보다 엄청 빠르다.
 * 
 ...
 * 
 * 두 클래스의 장점을 모두 이용해 조합하여 사용하는 방법도 가능하다. 처음에 작업하기 전에 데이터를 저장할 때는
 * ArrayList를 사용한 다음 작업할 때는 LinkedList로 데이터를 옮겨서 작업하면 더 나은 효율을 얻을 수 있다.
 */
...
//		return end - start;
	...
	public static long access(List list) {
		...
		for(int i = 0; i < 10000; i++) list.get(i); // 작업 시간
		long end = System.currentTimeMillis(); // 끝 측정
